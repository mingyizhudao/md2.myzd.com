...
var Jingle = J = {
    ...
    launch: function (opts) {
        ...
        if (this.settings.appType == 'single') {
            ...
            $(document).on('click', 'a[data-target="back"]', function (e) {
                e.preventDefault();
                window.history.go(-1);
            });
        }
...
J.Popup = (function ($) {
    ...
    TEMPLATE = {
         ...
         * confirm : '<div class="popup-title text-center color-000 font-type">{title}</div><div class="popup-content text-center">{content}</div><div id="popup_btn_container"><a class="cancel" data-icon="close">{cancel}</a><a data-icon="checkmark">{ok}</a></div>',
         * @type String
         *alert:'<div><img src="{title}" /></div>',点击图片，图片放大
         *confirm : '<div class="popup-title text-center color-000 font-type">{title}</div><div class="popup-content text-center">{content}</div><div id="popup_btn_container"><a data-target="closePopup">{cancel}</a><a href="mobilelogin" data-target="link">{ok}</a></div>',
         *确认退出跳转登录界面
         */
    ...
    var actionsheet = function (buttons) {
        ...
        /*
         ...
         <a href="#" class="font-type" style="color:red;">请名医住手给我回电话</a>
         </li>
         </ul>
         */
        ...
        $.each(buttons, function (i, n) {
            markup += '<ul class="list">' +
                    '<li class="text-center">' +
                    '<a href="' + n.href + '" data-target="' + n.target + '" class="font-type" style="color:' + n.color + ';">' + n.text + '</a>' +
                    '</li>';
        });
        markup += '</ul></div>';
...
(function ($) {
    ...
    var calendar = function (selector, options) {
        ...
        var _renderDay = function (date, month) {
            ...
            var classname = (_this.format(new Date()) == dateStr) ? 'active disabled' : '';
            if (date < _this.settings.date) {
                classname += ' disabled';
            }
    ...
    function slider(selector, noDots) {
        ...
        var _slide = function (i, duration) {
            ...
            if (index != i) {
                index = i;
                if (dots)
                    $(dots.find('li').get(index)).addClass('active').siblings().removeClass('active');
                afterSlide(index);
            }
        };
