...
function FileProgress(b,c){this.fileProgressID=b.id;this.file=b;this.opacity=100;this.height=0;this.fileProgressWrapper=$("#"+this.fileProgressID);if(!this.fileProgressWrapper.length){this.fileProgressWrapper=$("<tr></tr>");var j=this.fileProgressWrapper;j.attr("id",this.fileProgressID).addClass("progressContainer");var e=$("<td/>");e.addClass("progressName").text(b.name);var d=plupload.formatSize(b.size).toUpperCase();var f=$("<td/>");f.addClass("hide");f.addClass("progressFileSize").text(d);var i=$("<td/>");var a=$("<div/>");a.addClass("info");var g=$("<span class=sr-only />");g.text(d);var h=$("<a href=javascript:; />");h.show().addClass("progressCancel").text("×");a.append(h);i.append(a);j.append(e);j.append(f);j.append(i);$("#"+c).append(j)}else{this.reset()}this.height=this.fileProgressWrapper.offset().top;this.setTimer(null)}FileProgress.prototype.setTimer=function(a){this.fileProgressWrapper.FP_TIMER=a};FileProgress.prototype.getTimer=function(a){return this.fileProgressWrapper.FP_TIMER||null};FileProgress.prototype.reset=function(){this.fileProgressWrapper.attr("class","progressContainer");this.fileProgressWrapper.find("td .progress .progress-bar-info").attr("aria-valuenow",0).width("0%").find("span").text("");this.appear()};FileProgress.prototype.setChunkProgess=function(j){var k=Math.ceil(this.file.size/j);if(k===1){return false}var c=$('<button class="btn btn-default">查看分块上传进度</button>');var f=$('<tr class="chunk-status-tr"><td colspan=3></td></tr>');var g=$("<div/>");for(var d=1;d<=k;d++){var b=$('<div class="col-md-2"/>');var a=$('<div class="progress progress-striped"></div');var e=$("<div/>");e.addClass("progress-bar progress-bar-info text-left").attr("role","progressbar").attr("aria-valuemax",100).attr("aria-valuenow",0).attr("aria-valuein",0).width("0%").attr("id",this.file.id+"_"+d).text("");var h=$("<span/>");h.addClass("chunk-status").text();a.append(e);a.append(h);b.append(a);g.append(b)}if(!this.fileProgressWrapper.find("td:eq(2) .btn-default").length){this.fileProgressWrapper.find("td>div").append(c)}f.hide().find("td").append(g);f.insertAfter(this.fileProgressWrapper)};FileProgress.prototype.setProgress=function(p,e,o){this.fileProgressWrapper.attr("class","progressContainer green");var f=this.file;var b=f.loaded;var q=plupload.formatSize(b).toUpperCase();var l=plupload.formatSize(e).toUpperCase();var h=this.fileProgressWrapper.find("td .progress").find(".progress-bar-info");if(this.fileProgressWrapper.find(".status").text()==="取消上传"){return}this.fileProgressWrapper.find(".status").text("已上传: "+q+" 上传速度： "+l+"/s");p=parseInt(p,10);if(f.status!==plupload.DONE&&p===100){p=99}h.attr("aria-valuenow",p).css("width",p+"%");if(o){var n=Math.ceil(f.size/o);if(n===1){return false}var d=Math.ceil(b/o);var k,m;for(var i=0;i<d;i++){k=$("#"+f.id+"_"+i);k.width("100%").removeClass().addClass("alert-success").attr("aria-valuenow",100);m="块"+i+"上传进度100%";k.next().html(m)}var g=$("#"+f.id+"_"+d);var j;if(d<n){if(b%o){j=((b%o)/o*100).toFixed(2)}else{j=100;g.removeClass().addClass("alert-success")}}else{var c=f.size-o*(n-1);var a=f.size-b;if(a%c){j=((b%o)/c*100).toFixed(2)}else{j=100;g.removeClass().addClass("alert-success")}}g.width(j+"%");g.attr("aria-valuenow",j);m="块"+d+"上传进度"+j+"%";g.next().html(m)}this.appear()};FileProgress.prototype.setComplete=function(j,c){var d=this.fileProgressWrapper.find("td:eq(2)"),k=d.find(".progress");var l=$.parseJSON(c);var a;if(l.url){a=l.url;str="<div><strong>Link:</strong><a href="+l.url+" target='_blank' > "+l.url+"</a></div>"+"<div class=hash><strong>Hash:</strong>"+l.hash+"</div>"}else{var h=j.getOption("domain");a=h+encodeURI(l.key);var m=h+l.key;str="<div><strong>Link:</strong><a href="+a+" target='_blank' > "+m+"</a></div>"+"<div class=hash><strong>Hash:</strong>"+l.hash+"</div>"}k.html(str).removeClass().next().next(".status").hide();d.find(".progressCancel").hide();var g=this.fileProgressWrapper.find(".progressName");var f="?imageView2/1/w/100/h/100";var b=function(r){var t,u="";var p=["png","jpg","jpeg","gif","bmp"];var v=/\.([a-zA-Z0-9]+)(\?|\@|$)/;if(!r||!v.test(r)){return false}t=v.exec(r);u=t[1].toLowerCase();for(var s=0,q=p.length;s<q;s++){if(u===p[s]){return true}}return false};var e=b(a);var o=$('<a class="linkWrapper" target="_blank"/>');if(!e){}else{var i=new Image();if(!/imageView/.test(a)){a+=f}$(i).attr("src",a);var n=340;$(i).on("load",function(){showImg.attr("src",a);o.attr("href",a).attr("title","查看原图");function u(A,B,y){$("#myModal-img").modal();var z=$("#myModal-img").find(".modal-body");if(y<=300){$("#myModal-img").find(".text-warning").show()}var C=new Image();z.find("img").attr("src","images/loading.gif");C.onload=function(){z.find("img").attr("src",A).data("key",B).data("h",y);z.find(".modal-body-wrapper").find("a").attr("href",A)};C.src=A}var s=$('<div class="infoWrapper col-md-6"></div>');var t=$('<a class="fopLink"/>');t.attr("data-key",l.key).text("查看处理效果");s.append(t);t.on("click",function(){var A=$(this).data("key");var y=parseInt($(this).parents(".Wrapper").find(".origin-height").text(),10);if(y>$(window).height()-n){y=parseInt($(window).height()-n,10)}else{y=parseInt(y,10)||300}var B=[];B.push({fop:"imageView2",mode:3,h:y,q:100,format:"png"});B.push({fop:"watermark",mode:1,image:"http://www.b1.qiniudn.com/images/logo-2.png",dissolve:100,gravity:"SouthEast",dx:100,dy:100});var z=Qiniu.pipeline(B,A);$("#myModal-img").on("hide.bs.modal",function(){$("#myModal-img").find(".btn-default").removeClass("disabled");$("#myModal-img").find(".text-warning").hide()}).on("show.bs.modal",function(){$("#myModal-img").find(".imageView").find("a:eq(0)").addClass("disabled");$("#myModal-img").find(".watermark").find("a:eq(3)").addClass("disabled");$("#myModal-img").find(".text-warning").hide()});u(z,A,y);return false});var q=Qiniu.detectIEVersion();if(!(q&&q<=9)){var v=Qiniu.exif(l.key);if(v){var r=$('<a href="" target="_blank">查看exif</a>');r.attr("href",a+"?exif");s.append(r)}var x=Qiniu.imageInfo(l.key);var p=$("<div/>");var w='<div>格式：<span class="origin-format">'+x.format+"</span></div>"+'<div>宽度：<span class="orgin-width">'+x.width+"px</span></div>"+'<div>高度：<span class="origin-height">'+x.height+"px</span></div>";p.html(w);s.append(p)}Wrapper.append(s)}).on("error",function(){showImg.attr("src","default.png");Wrapper.addClass("default")})}};FileProgress.prototype.setError=function(){this.fileProgressWrapper.find("td:eq(2)").attr("class","text-warning");this.fileProgressWrapper.find("td:eq(2) .progress").css("width",0).hide();this.fileProgressWrapper.find("button").hide();this.fileProgressWrapper.next(".chunk-status-tr").hide()};FileProgress.prototype.setCancelled=function(b){var a="progressContainer";if(!b){a+=" red"}this.fileProgressWrapper.attr("class",a);this.fileProgressWrapper.find("td .progress").remove();this.fileProgressWrapper.find("td:eq(2) .btn-default").hide();this.fileProgressWrapper.find("td:eq(2) .progressCancel").hide()};FileProgress.prototype.setStatus=function(b,a){if(!a){this.fileProgressWrapper.find(".status").text(b).attr("class","status text-left")}};FileProgress.prototype.bindUploadCancel=function(a){var b=this;if(a){b.fileProgressWrapper.find("td:eq(2) .progressCancel").on("click",function(){b.setCancelled(false);b.setStatus("取消上传");b.fileProgressWrapper.find(".status").css("left","0");$(this).parents("tr").remove();if($("#fsUploadProgress").find("tr").length==0){$("#submitBtn").addClass("hide")}a.removeFile(b.file)})}};FileProgress.prototype.appear=function(){if(this.getTimer()!==null){clearTimeout(this.getTimer());this.setTimer(null)}if(this.fileProgressWrapper[0].filters){try{this.fileProgressWrapper[0].filters.item("DXImageTransform.Microsoft.Alpha").opacity=100}catch(a){this.fileProgressWrapper.css("filter","progid:DXImageTransform.Microsoft.Alpha(opacity=100)")}}else{this.fileProgressWrapper.css("opacity",1)}this.fileProgressWrapper.css("height","");this.height=this.fileProgressWrapper.offset().top;this.opacity=100;this.fileProgressWrapper.show()};
// Chinese (China) (zh_CN)
